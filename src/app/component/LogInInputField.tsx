  ...
  >
    <input
      className={`h-54px block px-2.5 pb-2.5 pt-4 text-sm bg-midnight rounded-lg border-1 outline-1 
          appearance-none dark:text-white dark:focus:border-teal focus:outline-none focus:ring-0 focus:border-teal peer transition-colors duration-300 ${className}`}
      type={type}
      name={name}
      value={value}
      onChange={onChange}
    ...
    <label
      ...
      className="absolute font-inter text-xs text-white-25 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] 
          bg-midnight dark:bg-midnight px-2 peer-focus:px-8 md:peer-focus:px-2 peer-focus:text-teal peer-focus:dark:text-teal peer-placeholder-shown:scale-100 
